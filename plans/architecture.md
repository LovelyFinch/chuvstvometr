...
| Хранилище | JSON-файл | Хранится один параметр — уровень чувств |
| Telegram-бот | node-telegram-bot-api | Популярная библиотека для Telegram Bot API |
...
graph TB
    ...
    subgraph VPS — один процесс Node.js
        BE[Express сервер]
        WS[WebSocket сервер]
        DB[JSON хранилище]
    ...
    BE -->|Чтение/запись| DB
    WS ---|Часть бэкенда| BE
    BOT -->|writeState + broadcastUpdate| DB
    BOT -->|broadcastUpdate| WS
...
### 3. Telegram-бот (backend/src/services/telegram.ts)

Сервис, встроенный в бэкенд-процесс:
- Работает в режиме long polling (не требует webhook)
- Принимает команды только от авторизованных пользователей (фильтр по Telegram user ID)
- Предоставляет inline-клавиатуру с 7 кнопками цветов для быстрой установки уровня
- Напрямую вызывает `writeState()` и `broadcastUpdate()` — без HTTP-запросов
- Опционален: если `TELEGRAM_BOT_TOKEN` не задан, бот не запускается

**Команды бота:**
- `/start` — приветствие + текущий уровень + клавиатура выбора
- `/level` — текущий уровень + клавиатура выбора
...
|---------|------|---------|----------|
| 1 | Синий | #2196F3 | Минимальный уровень |
| 2 | Зелёный | #4CAF50 | Нейтральный |
